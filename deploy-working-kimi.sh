...
echo "🔧 部署修正版Kimi API (带备用模拟响应)..."

# 上传修正的Kimi API文件
expect << 'EOF'
spawn scp server/services/kimi-working.js ubuntu@124.222.117.47:~/deploy-package/server/services/kimi-real.js
expect {
    "password:" {
...
expect << 'EOF'
spawn scp server/index-working.js ubuntu@124.222.117.47:~/deploy-package/server/index.js
expect {
    "password:" {
        send "Somkouny2016@g\r"
    }
