...
echo -e "${YELLOW}🔍 步骤2: 测试服务器连接${NC}"

# 测试SSH连接
ssh -o ConnectTimeout=10 -o BatchMode=yes $SERVER_USER@$SERVER_IP "echo '连接测试成功'" 2>/dev/null

if [ $? -eq 0 ]; then
    echo -e "${GREEN}✅ 服务器连接正常${NC}"
else
    echo -e "${RED}❌ 服务器连接失败，请检查网络和密码${NC}"
    exit 1
fi
...
echo -e "${YELLOW}🔍 步骤5: 在服务器上执行部署${NC}"
