...
echo "🔧 部署模拟Kimi API (稳定可用版本)..."

# 上传模拟Kimi API文件
expect << 'EOF'
spawn scp server/services/kimi-simulated.js ubuntu@124.222.117.47:~/deploy-package/server/services/kimi-real.js
expect {
    "password:" {
...
expect << 'EOF'
spawn scp server/index-simulated.js ubuntu@124.222.117.47:~/deploy-package/server/index.js
expect {
    "password:" {
        send "Somkouny2016@g\r"
    }
